...
export const useWebSocket = ({ id, callbacks, autoConnect = true }: UseWebSocketProps) => {
  const { socket, addListener, removeListener, sendMessage, forceUpdate } = useWebSocketContext();
  ...
  const stableCallbacks = useRef<WebSocketCallbacks>({
  ...
  useEffect(() => {
    if (autoConnect) {
      addListener(id, stableCallbacks.current);
    }

    return () => {
      removeListener(id);
    };
  }, [id, autoConnect, addListener, removeListener]);

  const sendWebSocketMessage = useCallback((message: unknown) => {
    sendMessage(message);
  }, [sendMessage]);

  return { socket, sendWebSocketMessage, forceUpdate };
}; 